...
**赛道五**提供的评测数据集基于赛道三提供的 YACLC-Minimal 和 YACLC-Fluency 进行构建，数据划分与赛道三相同。我们基于 BART-large 训练了基于 Seq2seq 结构的语法纠错模型，并将语法纠错模型在柱搜索解码过程中排名前五的生成结果作为待进行质量评估的语法纠错候选方案，以此构建语法纠错质量评估的训练集、开发集以及测试集。同时评测数据给出了训练集和开发集中每个语法纠错方案的真实 F<sub>0.5</sub> 分值。
