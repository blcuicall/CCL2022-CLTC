...
**赛道二**：中文语法错误检测（Chinese Grammatical Error Diagnosis）任务目的是检测出中文文本中每一处语法错误的位置、类型。语法错误的类型分为赘余(Redundant Words，R)、遗漏(Missing Words，M)、误用(Word Selection，S)、错序(Word Ordering Errors，W)四类。针对 M 和 S 类错误，给出纠正结果。如表 2 中所示，原句的第一个错误是位置为第 6 的字“了”，错误类型为 S，即误用，应该改为“理”（即将“了解”改成了“理解”）；第二个错误是位置为 8 的词“这”，错误类型为 R，即赘余。
...
<tr>
<td> 语法错误检测  </td>
    <td>00038800481, 6, 6, S, 理 </br> 00038800481, 8, 8, R </td>
</tr>
</table>
